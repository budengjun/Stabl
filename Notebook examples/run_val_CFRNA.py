...
en_cv = GridSearchCV(
    en,
    param_grid={"C": np.logspace(-2, 1, 5), "l1_ratio": [0.5, 0.7, 0.9]},
    scoring="roc_auc",
    cv=chosen_inner_cv,
    n_jobs=-1,
)
...
sgl_cv = GridSearchCV(
    sgl,
    scoring="roc_auc",
    param_grid={"alpha": np.logspace(-2, 1, 10), "l1_ratio": [0.5, 0.7, 0.9]},
    cv=chosen_inner_cv,
    n_jobs=-1,
)
...
X_train, X_valid, y_train, y_valid, ids, task_type = data.load_cfrna(
    "../Sample Data/CFRNA", percentile=None
)
