...
lasso_cv = GridSearchCV(
    lasso,
    param_grid={"C": np.logspace(-3, 0, 30)},
    scoring="roc_auc",
    cv=chosen_inner_cv,
    n_jobs=-1,
)
...
en_params = {"C": np.logspace(-3, 0, 10), "l1_ratio": [0.5, 0.7, 0.9]}
en_cv = GridSearchCV(
...
sgl_cv = GridSearchCV(
    sgl,
    scoring="roc_auc",
    param_grid={"alpha": np.logspace(-3, 0, 10), "l1_ratio": [0.5, 0.7, 0.9]},
    cv=chosen_inner_cv,
    n_jobs=-1,
)
...
stabl_alasso = clone(stabl).set_params(
    base_estimator=alasso, lambda_grid={"C": np.linspace(0.004, 4, 30)}, verbose=1
)
...
stabl_sgl = clone(stabl).set_params(
    ...
    lambda_grid=[
        {"alpha": np.logspace(-3, -1, 5), "l1_ratio": [0.5]},
        {"alpha": np.logspace(-3, -1, 5), "l1_ratio": [0.7]},
        {"alpha": np.logspace(-3, -1, 5), "l1_ratio": [0.9]},
    ],
    verbose=1,
)
