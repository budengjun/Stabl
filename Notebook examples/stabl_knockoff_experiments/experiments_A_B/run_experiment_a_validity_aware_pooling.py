#!/usr/bin/env python3
"""Experiment A V6.1: validity-aware multiple-completion pooling for STABL.
...
for real features. V6.1 compares three downstream rules:

1. ``mean_path_naive``: the V6 negative control that also averages artificial
   score paths by identity.
2. ``mean_real_component_artificial_count``: the primary validity-aware rule;
   artificial evidence is aggregated through the mean component-wise exceedance
   count at each threshold.
3. ``component_selection_vote``: a sensitivity rule that votes across the
   component-level original-STABL sets and does not claim an FDP+ estimate.
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", default="./experiment_A_validity_aware_pooling_v61")
    ...
    parser.add_argument(
        ...
        help="Validity-aware rule is primary; naive path pooling is a negative control.",
    )
    parser.add_argument(
        "--vote-fractions",
        default="0.5,0.8",
        help="Vote fractions used only by component_selection_vote.",
    )
    parser.add_argument(
        "--stabl-seed-mode",
    ...
    config: dict[str, Any] = {
        "experiment": "A_validity_aware_multiple_posterior_pooling",
        "version": "6.1",
        **vars(args),
        ...
        "selection_rules_resolved": pooling_rules,
    ...
    pooled_path = out_dir / "validity_aware_selection_results.csv"
            ...
            for completion in completions:
                ...
                for config_index, generator_config in enumerate(generator_configs):
                    ...
                    component_results = [
                        apply_selection_rule(
                            "stabl_min",
                            real_components[index],
                            artificial_components[index],
                            q=args.target_fdr,
                            threshold_grid=threshold_grid,
                        )
                    ...
                    for pool_size in pool_sizes:
                        for aggregator in aggregators:
                            ...
                            if "mean_path_naive" in pooling_rules:
                                naive_result = apply_selection_rule(
                                    "stabl_min",
                                    pooled_real,
                                    pooled_artificial,
                                    q=args.target_fdr,
                            ...
                            if "mean_real_component_artificial_count" in pooling_rules:
                                ...
                                threshold_diagnostics = curve_value_at_selected_threshold(validity)
                                rule_results.append((
                            ...
                            if "component_selection_vote" in pooling_rules:
                                for vote_fraction in vote_fractions:
                                    vote_result = select_component_vote(
                                        component_masks,
                                        pool_size=pool_size,
                            ...
                            for pooling_rule, vote_fraction, result, extra, curve_payload in rule_results:
                                row: dict[str, Any] = {
                                    ...
                                    "mean_component_estimated_fdp": float(
                                        np.mean(component_estimated_fdps[:pool_size])
                                    ),
                                    "mean_component_selected_count": float(
                                        np.mean(component_selected_counts[:pool_size])
                                    ),
                                ...
                                if args.save_pooled_scores:
                                    ...
                                    atomic_savez_compressed(
                                        ...
                                        / (
                                            f"rep_{replicate:04d}__{mechanism}__{completion}__"
                                            f"{generator_config.label}__M_{pool_size:03d}__{aggregator}__"
                                ...
                                print(
                                    ...
                                    f"rule={pooling_rule} vote={vote_fraction} "
    ...
    print(f"\nExperiment A V6.1 completed. Results: {out_dir}", flush=True)
