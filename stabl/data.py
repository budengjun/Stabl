...
def load_onset_of_labor(data_path):
    ...
    patients_id = pd.read_csv(join(data_path, "Training", "ID.csv"), index_col=0).Id

    # importing the training data
    y_train = pd.read_csv(join(data_path, "Training", "DOS.csv"), index_col=0).DOS
    cyto_train = pd.read_csv(join(data_path, "Training", "CyTOF.csv"), index_col=0)
    prot_train = pd.read_csv(join(data_path, "Training", "Proteomics.csv"), index_col=0)

    # importing the validation data
    y_valid = pd.read_csv(
        join(data_path, "Validation", "DOS_validation.csv"), index_col=0
    ).DOS
    ...
    train_data_dict = {"CyTOF": cyto_train, "Proteomics": prot_train}

    valid_data_dict = {"CyTOF": cyto_valid, "Proteomics": prot_valid}
...
def load_dream(data_path):
    ...
    patients_id = pd.read_csv(
        join(data_path, "Patients_id.csv"), index_col=0
    ).participant_id
    ...
    train_data_dict = {"Phylotype": phylo_train, "Taxonomy": taxo_train}
...
def load_covid_19(data_path):
    ...
    y_train = pd.read_csv(
        join(data_path, "Training", "Mild&ModVsSevere.csv"), index_col=0
    ).iloc[:, 0]

    # Importing the validation data
    X_val = pd.read_csv(
        join(data_path, "Validation", "Validation_proteomics.csv"), index_col=0
    )
    y_val = ~pd.read_csv(
        join(data_path, "Validation", "Validation_outcome(WHO.0 >= 5).csv"), index_col=0
    ).iloc[:, 0]

    X_train = {"Proteomics": X_train}
    X_val = {"Proteomics": X_val}
    task_type = "binary"
...
def load_ssi(data_path):
    ...
    # importing the training data
    y_train = pd.read_csv(join(data_path, "outcome.csv"), index_col=0).model1b
    cyto_train = pd.read_csv(join(data_path, "CyTOF.csv"), index_col=0)
    prot_train = pd.read_csv(join(data_path, "Proteomics.csv"), index_col=0)
