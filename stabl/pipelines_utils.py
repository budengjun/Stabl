    ...
    for name, predictions in predictions_dict.items():
        ...
        if isinstance(predictions, pd.DataFrame):
            ...
            pred_to_save = pd.DataFrame(
                np.asarray(predictions).ravel(), columns=["prediction"]
            )
        ...
        pd.concat([pred_to_save, y_to_save], axis=1).to_csv(
        ...
        predictions_clean, y_clean = clean_predictions_for_plots(
            predictions=predictions, y=y, model_name=name
        )
        ...
        if task_type == "binary":
            ...
            boxplot_binary_predictions(
                ...
                show_fig=False,
                paths=os.path.join(
                    saving_path, f"{name} Boxplot of median predictions.pdf"
                ),
            )
            ...
            scatterplot_regression_predictions(
                ...
                show_fig=False,
                paths=os.path.join(
                    saving_path, f"{name} Scatter-plot of median predictions.pdf"
                ),
            )


def compute_scores_table(
    predictions_dict, y, task_type="binary", selected_features_dict=None
):
    ...
    def clean_predictions_for_scores(preds, y, model_name=None):
        ...
        if valid_mask.sum() < len(valid_mask):
            ...
            print("Number of samples:", len(valid_mask))
            print(
                "Samples dropped because prediction is NaN:",
                len(valid_mask) - valid_mask.sum(),
            )
    ...
    for model, preds in predictions_dict.items():
        y_array, preds_array = clean_predictions_for_scores(
            preds=preds, y=y, model_name=model
        )
        ...
        for metric in scores_columns:
            if metric == "ROC AUC":
                ...
                cell_value = (
                    f"{model_roc:.3f} [{model_roc_CI[0]:.3f}, {model_roc_CI[1]:.3f}]"
                )

            elif metric == "Average Precision":
                model_ap = average_precision_score(y_array, preds_array)
                model_ap_CI = compute_CI(
                    y_array, preds_array, scoring="average_precision"
                )
                cell_value = (
                    f"{model_ap:.3f} [{model_ap_CI[0]:.3f}, {model_ap_CI[1]:.3f}]"
                )
                ...
                iqr_features = np.quantile(sel_features, [0.25, 0.75])
                ...
                jaccard_iqr = np.quantile(jaccard_val, [0.25, 0.75])
                cell_value = (
                    f"{jaccard_median:.3f} [{jaccard_iqr[0]:.3f}, {jaccard_iqr[1]:.3f}]"
                )
                ...
                cell_value = (
                    f"{model_r2:.3f} [{model_r2_CI[0]:.3f}, {model_r2_CI[1]:.3f}]"
                )
                ...
                cell_value = (
                    f"{model_rmse:.3f} [{model_rmse_CI[0]:.3f}, {model_rmse_CI[1]:.3f}]"
                )
                ...
                cell_value = (
                    f"{model_mae:.3f} [{model_mae_CI[0]:.3f}, {model_mae_CI[1]:.3f}]"
                )
            else:
                continue
...
def compute_pvalues_table(
    ...
    def clean_pair_predictions_for_pvalues(
        preds, preds2, y, model_name=None, model2_name=None
    ):
        """
        ...
        if isinstance(y, pd.Series):
            common_index = preds_clean.index.intersection(
                preds2_clean.index
        ...
        valid_mask = (
            ~np.isnan(y_array) & ~np.isnan(preds_array) & ~np.isnan(preds2_array)
        )
    ...
    p_values_dict = {
        s: pd.DataFrame(columns=predictions_dict.keys(), index=predictions_dict.keys())
    ...
    for metric in scores_columns:
        ...
        for model, preds in predictions_dict.items():
            for model2, preds2 in predictions_dict.items():
                if metric == "ROC AUC":
                    y_array, preds_array, preds2_array = (
                        clean_pair_predictions_for_pvalues(
                            preds=preds,
                            preds2=preds2,
                            y=y,
                            model_name=model,
                    ...
                    p_value = permutation_test_between_clfs(
                        y_array, preds_array, preds2_array, scoring="roc_auc"
                    )[1]
                    ...
                    p_value = permutation_test_between_clfs(
                        y_array, preds_array, preds2_array, scoring="average_precision"
                    )[1]

                elif metric == "N features":
                    sel_features = selected_features_dict[model]["Fold nb of features"]
                    sel_features2 = selected_features_dict[model2][
                        "Fold nb of features"
                    ]

                    p_value = mannwhitneyu(x=sel_features, y=sel_features2).pvalue
                    ...
                    jaccard_val2 = jaccard_mat2[np.triu_indices_from(jaccard_mat2, k=1)]

                    p_value = mannwhitneyu(x=jaccard_val, y=jaccard_val2).pvalue
                    ...
                    p_value = mannwhitneyu(x=preds_array, y=preds2_array).pvalue
...
def compute_features_table(
    ...
    if task_type == "binary":
        df_out["Train Mannwithney pvalues"] = [
            mannwhitneyu(
        ...
        df_out["Train Pearson-r pvalues"] = [
            stats.pearsonr(
        ...
        df_out["Train Spearman-r pvalues"] = [
            stats.spearmanr(
                X_train.loc[:, i].dropna(),
                y_train.loc[X_train.loc[:, i].dropna().index],
            )[1]
    ...
    if X_test is not None:
        if task_type == "binary":
            df_out["Test Mannwithney pvalues"] = [
                mannwhitneyu(
            ...
            df_out["Test Spearman-R pvalues"] = [
                stats.spearmanr(
                    X_test.loc[:, i].dropna(),
                    y_test.loc[X_test.loc[:, i].dropna().index],
                )[1]
...
class BenchmarkWrapper:
        ...
        if predict is None:
            if use_predict_proba:
                if hasattr(self.model, "predict_proba"):
                    ...
                    def predict_proba(x):
                        return getattr(self.model, "predict_proba")(x)[:, 1].flatten()
    ...
    def _get_importances(self):
        ...
        try:
            ...
            raise NotImplementedError(
                "The model does not have the method 'get_importances' \
    ...
    def _get_support(self, indices=False):
        ...
        scores = self.get_importances()  # pylint: disable=no-member
